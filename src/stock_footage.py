...
from tmp import create_directories_from_path, tmp_content_scene_dir_path, tmp_scene_path
import os
import subprocess
...
def check_media_duration(audio_filename):
    result = subprocess.run(["ffprobe", "-v", "error", "-show_entries",
                            "format=duration", "-of",
                            "default=noprint_wrappers=1:nokey=1", audio_filename],
...
def generate_stock_footage(paper_id: str, script: Script):
    def _process_scene(context: tuple[int, int], scene: TextScriptScene):
      ...
      audio_filename = tmp_scene_path(paper_id=paper_id, section_id=context[0], scene_id=context[1], kind="audio")
