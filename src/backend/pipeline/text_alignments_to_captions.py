...
from .script import TextScriptScene, Script, for_every_scene
...
class SmallGrowFromCenter(GrowFromCenter):
    def interpolate_mobject(self, alpha: float) -> None:
        ...
        self.mobject.scale(
            scale_factor * 0.1
        )  # Adjust this value to change the growth amount
...
class WordOverlay(Scene):
    ...
    def construct(self):
        ...
        alignment_path = tmp_scene_path(
            paper_id, section_id, scene_id, "text_alignments"
        )
        ...
        for i, fragment in enumerate(data["fragments"]):
            ...
            text_fragment = re.sub(
                r"^\W+|\W+$", "", text_fragment
            )  # remove non-alphanumeric characters at the start and end of the string
